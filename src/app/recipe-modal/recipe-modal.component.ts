...
export class RecipeModalComponent implements OnInit {
  ...
  saveEdits() {
    ...
    var has_admin =
      this.userService.user && this.userService.user.adminflag == true;

    if (has_admin) {
      this.commonService.editRecipe(this.recipe_full).subscribe(
        (data) => {
          console.log('edited');
        },
        (error) => console.error(error)
  ...
  makePrimary(image) {
    // console.log(image)
    this.commonService.makePrimary(image.id, image.recipe_id).subscribe(
      (data) => {
        // this.recipe_comments = data;
      },
      (error) => console.log(error)
